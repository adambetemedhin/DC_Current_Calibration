...
def wdatacollect(wDATA,ltime,cstep,setc,cvolt,cur,sv):
        wDATA.append(ltime + ',' + str(cstep) + ',' + str(setc) + ',' + str(float(cvolt)) + ',' + str(cur) + ',' + str(sv))
        return wDATA
...
def writedata(tofile):
        if os.path.exists('DATA.csv'):
                ...
                method = 'w'
                header=[]
                header.append('TIMESTAMP,DESIRED CURRENT,SET CURRENT,VOLTAGE,MEASURED CURRENT,SHUNT VOLTAGE DROP')
                datafile = open('DATA.csv', method)
                for item in header:
                        datafile.write("%s\n" % item)
                datafile.close()
...
if configexists == 1:
        ...
        if csettings == 2:
                ...
                if connection_method == 1:
                        ...
                        DLM_port = input('What is the port for the DLM40-15 (Default:9221)? ')
                ...
                tdelay = input('What is the desired delay (dwell) time for each step (in seconds, make larger than 5)?: ')
